...
<body>
    ...
    <script>
        ...
        // Debug: Log config
        console.log('Board config loaded:', boardConfig);
        ...
        if (boardConfig.notifications.enabled && 'Notification' in window) {
            console.log('Notifications are enabled, checking permission...');
            notificationsPermission = Notification.permission;
            console.log('Current permission:', notificationsPermission);
                ...
                Notification.requestPermission().then(permission => {
                    ...
                    if (permission === 'granted') {
                        ...
                        setTimeout(() => {
                            ...
                            showNotification('Flatban Notifications Enabled', 'You will now receive notifications for board updates');
                        }, 1000);
                    }
                });
            } else if (notificationsPermission === 'granted') {
                console.log('Permission already granted');
                // TEST: Show a test notification on page load
                setTimeout(() => {
                    console.log('Showing test notification on page load...');
                    showNotification('Flatban Board Loaded', 'Notifications are working! You will be notified of changes.');
                }, 1000);
            }
        } else {
            console.log('Notifications not enabled or not supported');
            console.log('- Enabled:', boardConfig.notifications.enabled);
        ...
        function showNotification(title, body, icon = '📋') {
            ...
            if (notificationsPermission === 'granted') {
                ...
                console.log('Notification permission not granted. Current permission:', notificationsPermission);
            }
        }

        function shouldNotify(updateData) {
            console.log('shouldNotify called with:', updateData);
            ...
            // TEMPORARY: Skip focus check for testing
            ...
            if (boardConfig.notifications.all_changes) {
                console.log('All changes enabled, will notify');
                return true;
            }

            // Check if we should notify for specific column moves
            if (updateData.action === 'move' && updateData.toColumn) {
                const shouldNotify = boardConfig.notifications.notify_columns.includes(updateData.toColumn);
                console.log('Column-specific notification:', shouldNotify, 'for column:', updateData.toColumn);
                return shouldNotify;
            }

            // Don't notify for generic filesystem changes unless all_changes is enabled
            if (updateData.action === 'filesystem_change') {
                console.log('Filesystem change but all_changes not enabled');
                return false;
            }

            console.log('No notification rule matched');
            return false;
        }
        ...
        function connectSSE() {
            ...
            eventSource.onmessage = (event) => {
                try {
                    ...
                    if (data.type === 'update') {
                        console.log('Board updated via SSE');

                        // Check if we should show a notification
                        const willNotify = shouldNotify(data);
                        console.log('Will notify?', willNotify);
                        ...
                        if (willNotify) {
                            ...
                            } else if (data.action === 'filesystem_change') {
                                notificationTitle = 'Board Updated';
                                notificationBody = 'A task was created, modified, or updated';
                            } else {
