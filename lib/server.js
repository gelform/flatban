...
function createServer(boardPath, port) {
    ...
    let ignoreNextWatch = false; // Flag to ignore filesystem events from API operations
    ...
    try {
        fs.watch(watchPath, { recursive: true }, (eventType, filename) => {
            ...
            // Ignore if this was triggered by an API operation
            if (ignoreNextWatch) {
        ...
        if (req.method === 'POST' && req.url === '/api/move') {
            ...
            req.on('end', () => {
                try {
                    ...
                    // Ignore filesystem watcher for this change
...
<body>
    ...
    <script>
        ...
        function shouldNotify(updateData) {
            ...
            // Skip if this is the active tab (user can see the change)
                    ...
                    if (data.type === 'update') {
                        ...
                        if (shouldNotify(data)) {
                            ...
                            if (data.action === 'move') {
                                ...
                                const shortId = data.taskId.substring(0, 7);
                                notificationBody = \`\${shortId}: "\${data.taskTitle}" → \${data.toColumnName}\`;
                            } else if (data.action === 'delete') {
                                notificationTitle = 'Task Deleted';
                                const shortId = data.taskId.substring(0, 7);
                                notificationBody = \`\${shortId}: "\${data.taskTitle}"\`;
