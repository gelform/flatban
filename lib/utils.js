...
function parseSimpleYaml(content) {
    ...
    for (const line of lines) {
        ...
        if (currentSection === null && /^name:\s*"?([^"]+)"?/.test(trimmed)) {
            ...
            if (/^\s*enabled:\s*(\S+)/.test(trimmed)) {
                const match = trimmed.match(/^\s*enabled:\s*(\S+)/);
                config.notifications.enabled = match[1] === 'true';
            } else if (/^\s*all_changes:\s*(\S+)/.test(trimmed)) {
                const match = trimmed.match(/^\s*all_changes:\s*(\S+)/);
                config.notifications.all_changes = match[1] === 'true';
            } else if (/^\s*notify_columns:\s*\[([^\]]*)\]/.test(trimmed)) {
                const match = trimmed.match(/^\s*notify_columns:\s*\[([^\]]*)\]/);
                const columnsStr = match[1].trim();
                config.notifications.notify_columns = columnsStr ? columnsStr.split(',').map(c => c.trim()) : [];
            }
        }
    }
